...
**P2 长尾推进（2026-06-03 中午）：**

| # | 任务 | Commit | 说明 |
|---|------|--------|------|
| P2-11 | 报告价差提示 | `687ff94` | /api/reports/price-drift + PriceDriftBanner（≥2% 或 24h+ 琥珀警示；无结构化价格时诚实降级为时效提示） |
| P2-12 | 质量徽章 | `687ff94` | QualityBadge：pass绿/warn琥珀/block红，报告顶部一眼可见 |
| P2-6 | 半成品处置 | `d908340` | 回测收益曲线（回撤区间+持仓区间+成交表）+ A股面板进主导航(/cn-market)；告警核实已是真数据无需改 |

**P2 最终未做项**（留待后续会话，均为 L 工作量）：P2-5 英文 claim 中文化 /
P2-7 成本审计面板 / P2-8 agent 图表 / P2-9 移动端响应式 / P2-10 A股体验补齐

**P2 长尾全部完成（2026-06-03 下午）：**

| # | 任务 | 说明 |
|---|------|------|
| 图表智能选型 ⭐ | LLM 决策替换关键词匹配（实测 6/6 全对：营收构成→pie/RSI→candlestick/寒暄→不出图）；修复推理模型适配（2048 tokens/45s超时/think标签剥离）+ fallback 倒挂 |
| P2-8 agent 图表 | technical(K线/MA/RSI仪表盘) + macro(横截面柱状) + risk(雷达/仪表盘) |
| P2-7 成本审计 | cost_audit.db 持久化 + /api/system/cost-audit + /cost-audit 页面（每日趋势/Top消耗） |
| P2-5 中文化 | 5 个 agent 全部英文模板中文化 + 枚举映射 + 6 个契约测试守护 |
| P2-10 A股体验 | ~100 个中文公司名映射 + KDJ 指标 + ¥/万亿单位 + T+1 制度提示条 |
| P2-9 移动端 | 44px 触摸目标 + 7 处表格横向滚动（抽屉导航/底部面板等基础设施已存在） |

**P2 完成度：12 项全部完成** ✅（P2-1~P2-12 + 图表智能化加餐）
...
## 九、P2 长尾全部完成后的最终验证（2026-06-03 下午）

- **后端全量回归：1769 passed / 8 skipped / 0 failed / 0 errors**（10 分 18 秒）
- **前端全量：159 passed（28 文件）** + 构建绿
- **累计 commit：36 个**（基线 23ef149，全部未 push）

### 测试数量演进（两天的工作量）

| 时间点 | 后端测试 | 前端测试 |
|--------|---------|---------|
| 冲刺开始前 | 1497 passed + 1 failed + 41 errors | 68 |
| 昨晚结束（P0/P1/工作台） | 1631 passed / 0 failed | 91 |
| **现在（P2 全部 + agent 联动 + 图表智能化）** | **1769 passed / 0 failed** | **159** |

净增 272 个后端测试 + 91 个前端测试，全部伴随真实功能。

### 期间发现并修复的回归（1 个）

成本审计的 SystemRouterDeps 必需字段破坏了 6 个现有测试 → 改为可选字段（向后兼容设计），
commit 3d99228。
