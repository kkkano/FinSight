...
- 已完成：`prepare_context` 主链路合并、`understand_request` 主链路接入、`GraphState` 新增理解层字段、`trace/visibility=user`、前端 trace 消费、policy task 工具并集、planner stub 多任务消费、executor `task_results` 归集、多任务 fallback 简报、20 query probe、Playwright 聊天 UX smoke、后端 conversation API、服务端 conversation snapshot store、会话标题/messages/PATCH、删除会话时清理 session context/report index/thread RAG collections/RAG observability runs、前后端停止生成闭环、executor/agent cooperative cancellation token。
