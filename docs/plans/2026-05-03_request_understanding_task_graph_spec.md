...
- 已完成：`understand_request` 主链路接入、`GraphState` 新增理解层字段、`trace/visibility=user`、前端 trace 消费、policy task 工具并集、planner stub 多任务消费、多任务 fallback 简报、20 query probe、Playwright 聊天 UX smoke。
- 保留兼容：旧 `chat_respond / resolve_subject / clarify / parse_operation` 仍注册，用于兼容测试和少数回退逻辑。
- 待完成：`prepare_context` 合并、planner/executor/synthesize 全量多任务 PlanIR 原生化、后端 conversation API、端到端 cancel trace 完整落库。
