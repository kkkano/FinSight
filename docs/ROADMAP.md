...
### P1????????
- **P1-34 ??????**?????10-30??+ ?????? + ???/??/???? + JSON/MD ?????????????? Mock?5 ??????????????
- **P1-31 ReAct ????**??????? + ?? + ??????? N ?????????????????? ?20%??????????? <5%??????????
- **P1-Trace ?????**?TraceEvent schema ???event_type/duration/metadata?+ ?????????????????? trace?????????
...
| # | 任务 | 交付物 | 验收标准 |
|---|------|--------|----------|
| **34** | 回归评估基线 | `tests/regression/` 基准集(10-30条) + 一键脚本 | 覆盖率/引用/耗时统计；JSON/MD对比报告；CI<5分钟 |
| **31** | ReAct 搜索收敛 | 信息增益评分 + 去重 + 停止条件 | 轮次降≥20%；引用不降；回归不退化 |
| **Trace** | 可观测性统一 | TraceEvent schema(event_type/duration/metadata) | 前端统一展开 trace 详情 |
