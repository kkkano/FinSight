...
## 条目 3：Report 视觉对齐 design_concept_v2
- 时间：2026-01-11
- 目标：ReportView 结构与样式对齐设计稿，提升报告可信度
- 变更：
  - ReportView 标题/摘要/段落/图表布局对齐
  - Recommendation 模块样式增强
  - 样式参考 design_concept_v2.html 并同步色彩/间距
- 测试：
  - `npm run test`
  - 结果：No frontend tests yet - smoke only
- 当前项目情况：
  - Report UI 与设计稿对齐，可作为后续迭代基础
- 下一步动作：
  1) MacroAgent 升级（FRED）
  2) 向量 RAG 基础（LlamaIndex + Chroma）
  3) Agent 进度指示器
## 条目 4：Report 结构化输出增强 + Watchlist + Charts
- 时间：2026-01-11
- 目标：补齐 Report 结构化输出、Watchlist 与图表能力
- 变更：
  - PDF 导出接入 Watchlist/关注资产展示
  - ReportContent.chart 对接 ECharts 规范
  - 报告结构与前端渲染字段对齐
- 测试：
  - `npm run test`
  - 结果：No frontend tests yet - smoke only
- 当前项目情况：
  - Report 结构化输出更完整，前后端对齐
- 下一步动作：
  1) MacroAgent 升级（FRED）
  2) 向量 RAG 基础（LlamaIndex + Chroma）
  3) Agent 进度指示器
...
## 条目 7：性能对比 + DeepSearch 追溯 + 引用对齐
- 时间：2026-01-11
- 目标：补齐 YTD/1Y 表现对比，DeepSearch 引用/trace 与报告对齐
- 变更：
  - get_performance_comparison 统一走 get_stock_historical_data
  - analyze_historical_drawdowns 输出结构化序列
  - ReportHandler 对接 DeepSearchAgent 证据/trace 并写入 ReportIR citations/meta
  - 完善 Markdown 渲染与 message.md 输出（Reasoning Trace + Sources）
- 测试：未运行
- 当前项目情况：
  - 前端可展示 trace + citations，报告可信度提升
...
## 条目 17：Agent 进度指示器
- 时间：2026-01-12
- 目标：在流式输出时显示各 Agent 实时状态，减少用户等待焦虑
- 变更：
  1. **ThinkingProcess.tsx 增加 Agent stage 标签** (`frontend/src/components/ThinkingProcess.tsx:18-24`)
     - 新增 supervisor_start/agent_start/agent_done/agent_error/forum_start/forum_done 标签
     - 使用 emoji 图标增强可读性
  2. **client.ts 处理 Agent 进度事件** (`frontend/src/api/client.ts:217-225`)
     - 识别 supervisor_start/agent_start/agent_done/agent_error/forum_start/forum_done 事件
     - 转换为 thinking 格式传递给 onThinking 回调
  3. **getStageIcon 函数优化** (`frontend/src/components/ThinkingProcess.tsx:27-32`)
     - 使用 Unicode 符号替代文字图标
- 测试：
  - 语法检查通过
  - 建议手动测试：发送 REPORT 意图请求，观察 Reasoning trace 中的 Agent 进度
- 当前项目情况：
  - Agent 进度指示器已完成，用户可在 Reasoning trace 中看到各 Agent 执行状态
  - 后端 Supervisor 已发送完整的 Agent 进度事件，无需修改

## 条目 18：MacroAgent 升级 - 集成 FRED API
- 时间：2026-01-12
- 目标：MacroAgent 从 Mock 数据升级为真实 FRED API 数据
- 变更：
  1. **tools.py 新增 FRED API 支持** (`backend/tools.py:75, 2297-2389`)
     - 新增 `FRED_API_KEY` 环境变量配置
     - 新增 `get_fred_data()` 函数获取宏观经济数据
     - 支持指标：CPI/联邦基金利率/GDP增长率/失业率/10年期国债/收益率曲线
     - 收益率曲线倒挂自动警告
  2. **MacroAgent 重构** (`backend/agents/macro_agent.py`)
     - `_initial_search()` 调用 `get_fred_data()` 获取真实数据
     - 搜索回退机制：FRED API 失败时使用 web search
     - `_first_summary()` 生成结构化宏观数据摘要
     - `_format_output()` 添加多项证据和风险提示
- 测试：
  - 语法检查通过
  - 建议手动测试：问 "当前宏观经济形势" 或 "美联储利率" 验证 FRED 数据
- 当前项目情况：
  - MacroAgent 已从 Mock 升级为真实 FRED API
  - 支持 6 项核心宏观指标
  - 无 API key 时自动使用估计值回退

## 条目 19：向量 RAG 基础 - ChromaDB + Sentence Transformers
- 时间：2026-01-12
- 目标：实现向量检索增强生成基础架构，为 DeepSearch 和用户记忆提供向量化能力
- 变更：
  1. **requirements.txt 新增依赖** (`requirements.txt:75-77`)
     - chromadb>=0.4.0 (向量数据库)
     - sentence-transformers>=2.2.0 (本地 Embedding)
  2. **新建 knowledge 模块** (`backend/knowledge/`)
     - `__init__.py`: 模块导出
     - `vector_store.py`: ChromaDB 单例封装，支持持久化和内存模式
     - `rag_engine.py`: RAG 引擎，封装切片、入库、检索
  3. **VectorStore 核心功能**
     - 延迟加载 ChromaDB 和 SentenceTransformer
     - 支持临时集合（ephemeral）和持久化集合
     - 使用 paraphrase-multilingual-MiniLM-L12-v2 多语言模型
     - add_documents/query/delete_collection 完整 CRUD
  4. **RAGEngine 核心功能**
     - chunk_text(): 智能切片，支持句子边界和重叠
     - ingest_document(): 文档切片并入库
     - query(): Top-K 相似度检索
     - query_with_context(): 返回格式化上下文字符串
     - create_ephemeral_collection(): 创建临时工作台
- 测试：
  - 语法检查通过
  - 建议手动测试：`from backend.knowledge import VectorStore, RAGEngine`
- 当前项目情况：
  - 向量 RAG 基础架构已完成
  - 可用于 DeepSearchAgent 文档向量化和用户长期记忆
  - 下一步：集成到 DeepSearchAgent
