...
### 1.2 RAG 框架：LlamaIndex
**决策理由**:
1.  **数据处理强**: 在解析 PDF、复杂表格、长研报方面，LlamaIndex 的 Data Loader 比 LangChain 原生工具更专业。
2.  **查询灵活**: 支持 "Summary Index" (总结全文) 和 "Vector Store Index" (精准检索) 的组合查询，非常契合研报分析场景。
...
│   ├── ingestion.py        # PDF/URL -> Chunks 处理流
│   └── rag_engine.py       # LlamaIndex 查询引擎封装
```
