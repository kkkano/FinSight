...
def test_scheduler_disabled_returns_none():
    scheduler = start_interval_scheduler(lambda: None, interval_minutes=0.01, enabled=False)
...
def test_scheduler_runs_job_at_least_once():
    ...
    scheduler = start_interval_scheduler(_job, interval_minutes=0.01, enabled=True)
