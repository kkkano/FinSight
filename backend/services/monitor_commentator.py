...
返回严格 JSON：{{"level":"info|warn|alert","text":"不超过120字","source":"agent","escalated":false,"prediction_id":null或给定id}}。
    ...
    *, generator: Callable[[str], Awaitable[dict[str, Any]]] | None = None, store=None,
    ...
    for trigger in triggers:
        try:
            ...
            allowed_prediction_id = prediction.id if prediction is not None else None
            if draft.prediction_id not in {None, allowed_prediction_id}:
                raise ValueError("commentator returned foreign prediction_id")
            prediction_id = draft.prediction_id
            source, level, text_value, escalated = "agent", draft.level, draft.text, draft.escalated
        except Exception as exc:
            prediction_id = None
            source, level, escalated = "system", "error", False
...
def produce_monitor_comments_sync(target, snapshot, triggers, prediction=None) -> bool:
    return asyncio.run(produce_monitor_comments(target, snapshot, triggers, prediction)) > 0
