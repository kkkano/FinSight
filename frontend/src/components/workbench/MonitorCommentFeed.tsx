...
export function MonitorCommentFeed({ sessionId }: { sessionId: string | null | undefined }) {
  ...
  const { comments, error, isAvailable, unavailable } = useMonitorCommentFeed(sessionId);
  ...
  return (
    <div className="border-b border-fin-border bg-fin-bg/40 px-3 py-3" data-testid="monitor-comment-feed">
      <div className="mb-2 flex items-center justify-between">
        ...
        {isAvailable && unavailable && <span className="text-2xs text-fin-danger">该数据源暂不可用</span>}
        {isAvailable && error && !unavailable && <span className="text-2xs text-fin-danger">连接已中断</span>}
      </div>
      {items.length === 0 ? (
