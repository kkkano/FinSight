...
<p align="center">
  <strong>面向投资新手与散户的 AI 分析助手 —— 用大白话提问，自动输出「基本面 + 技术面 + 新闻 + 风险」的结构化分析</strong>
</p>
...
**FinSight AI** 是面向**个人投资者与投资新手**的 AI 分析助手。很多人不是看不懂财报，而是没时间、没有专业工具，被机构研报和一堆术语挡在门外。FinSight 让你直接用大白话提问（例如「苹果现在值得买吗？」「特斯拉这周发生了什么？」），它会自动去抓实时数据、看新闻、读财报，最后输出一份覆盖**基本面、技术面、新闻/催化剂、风险**的结构化分析，且每个结论都带可追溯的证据来源。

底层它是一个基于 **LangGraph** 的生产级多智能体系统，重点解决了 LLM 应用落地的可靠性问题：上游行情数据源不稳定、LLM 调用慢或超时，都不会拖垮整个流程（多源降级、熔断器、超时 + 确定性摘要兜底、模板报告回退）。
